...
function check_update()
	...
	os.remove(path)
	local url = 'https://github.com/KJLikuur/Oil-Tool.lua/raw/refs/heads/main/Update_Info.json'
...
if imgui.Button(fa.ROTATE_RIGHT .. u8" Перезагрузка ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.cfg.custom_dpi)) then
  ...
  imgui.OnFrame(function() return OilStats[0] end, function(player)
    imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 8, sizeY / 1.7), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
      ...
      imgui.CenterText(fa.CIRCLE_DOLLAR..u8'Заработано вирт: ' ..separator(settings.cfg.zarmon).. '$.')
      end 
      if settings.cfg.azcfg then 
      imgui.CenterText(fa.CIRCLE_DOLLAR..u8'Заработано AZ-coins:  '..settings.cfg.azzar.. 'шт.')
      end
      if settings.cfg.boshkamenu then 
      imgui.CenterText(fa.CIRCLE_DOLLAR..u8'Продано бочек: '..settings.cfg.boshkiini.. 'шт.')
      end 
      imgui.Separator()
...
if text:match('Вы успешно продали бочку') then
        ini.cfg.zarmon = settings.cfg.zarmon+boolGivemoney[0]
