...
function cfg_save()
inicfg.save(ini, "OilTools")
end
local objectcheck = false
local viborts = imgui.new.bool(ini.cfg.viborts)
local OilStats = imgui.new.bool(false)
local zarmonsesia = 0
local azzarsesia = 0
local boxhkosesia = 0
local moneycfg = imgui.new.bool(ini.cfg.moneycfg)
local azcfg = imgui.new.bool(ini.cfg.azcfg)
local skipdi = new.bool(ini.cfg.skipdi)
local boshkamenu = imgui.new.bool(ini.cfg.boshkamenu)
local boolGiveaz = imgui.new.int(ini.cfg.boolGiveaz)
local boolGivemoney = imgui.new.int(ini.cfg.boolGivemoney)
...
function downloadFileFromUrlToPath(url, path)
    ...
    imgui.GetIO().Fonts:AddFontFromMemoryCompressedBase85TTF(fa.get_font_data_base85('light'), 29, config, iconRanges)
        ...
        imgui.Begin('##Window', mainMenu,imgui.WindowFlags.NoTitleBar)
        ...
        if imgui.CustomButton(tab == 1, fa("screwdriver_wrench"), u8("Главная"), 0.80) then tab = 1 end
                if imgui.CustomButton(tab == 2, fa("screwdriver_wrench"), u8("Настройки"), 0.80) then tab = 2 end
        ...
        if tab == 1 then 
                ...
                imgui.Separator()
if imgui.Checkbox(u8' Заработанные вирты', moneycfg) then ini.cfg.moneycfg = moneycfg[0] cfg_save()  end
            if imgui.Checkbox(u8' Заработанные AZ-Coins', azcfg) then ini.cfg.azcfg = azcfg[0]  cfg_save() end 
                        if imgui.Checkbox(u8'Кол-во проданных бочек', boshkamenu) then ini.cfg.boshkamenu = boshkamenu[0]  cfg_save() end
         imgui.Separator()               
  		    		imgui.CenterText(u8'Заработано AZ-coins за всё время: '..ini.cfg.azzar.. 'шт.')
				    		imgui.CenterText(u8'Заработано AZ-coins за сессию: '..azzarsesia.. 'шт.')
		imgui.CenterText(u8'Заработано вирт за сессию: ' ..zarmonsesia.. '$.')
		imgui.CenterText(u8'Заработано вирт за всё время: ' ..ini.cfg.zarmon.. '$.')
	imgui.CenterText(u8'Продано бочек за сессию: '..boxhkosesia.. 'шт.')
	imgui.CenterText(u8'Продано бочек за всё время: '..ini.cfg.boshkiini.. 'шт.')
 ...
 imgui.BeginChild('##3', imgui.ImVec2(589 * MONET_DPI_SCALE, 87 * MONET_DPI_SCALE), true)
          ...
          apply_n_t()
            ...
            imgui.Text(fa.NOTE_STICKY..u8' Цвет MoonMonet') 
				...
				if imgui.Button(fa.ROTATE_RIGHT .. u8" Перезагрузка ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * ini.cfg.custom_dpi)) then
					reload_script = true
					thisScript():reload()
				end
	imgui.SameLine()
	if imgui.Button(fa.POWER_OFF .. u8" Выключение ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * ini.cfg.custom_dpi)) then
				...
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ##off', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar ) then
					if not isMonetLoader() then imgui.SetWindowFontScale(ino.cfg.custom_dpi) end
					imgui.CenterText(u8'Вы действительно хотите выгрузить (отключить) хелпер?')
					imgui.Separator()
					if imgui.Button(fa.CIRCLE_XMARK .. u8' Нет, отменить', imgui.ImVec2(200 * ini.cfg.custom_dpi, 25 * ini.cfg.custom_dpi)) then
						imgui.CloseCurrentPopup()
					end
					imgui.SameLine()
					if imgui.Button(fa.POWER_OFF .. u8' Да, выгрузить', imgui.ImVec2(200 * ini.cfg.custom_dpi, 25 * ini.cfg.custom_dpi)) then
						reload_script = true				
...
imgui.SameLine()
				if imgui.Button(fa.TRASH_CAN .. u8" Удаление ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * ini.cfg.custom_dpi)) then
				...
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ##delete', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
					if not isMonetLoader() then imgui.SetWindowFontScale(ini.cfg.custom_dpi) end
					...
					imgui.SameLine()
					if imgui.Button(fa.TRASH_CAN .. u8' Да, я хочу удалить', imgui.ImVec2(200 * ini.cfg.custom_dpi, 25 * ini.cfg.custom_dpi)) then
  ...
  imgui.OnFrame(function() return OilStats[0] end, function(player)
    imgui.SetNextWindowPos(imgui.ImVec2(500,500), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
       ...
       imgui.Begin('##Main OilStats', OilStats, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) 
      if ini.cfg.moneycfg then 
      imgui.CenterText(u8'Заработано вирт: ' ..ini.cfg.zarmon.. '$.')
      end 
      if ini.cfg.azcfg then 
      imgui.CenterText(u8'Заработано AZ-coins:  '..ini.cfg.azzar.. 'шт.')
      if ini.cfg.boshkamenu then 
      imgui.CenterText(u8'Продано бочек: '..ini.cfg.boshkiini.. 'шт.')
      end 
      end
      imgui.End()
      end)
...
if text:match('Вы успешно продали бочку') then
        ini.cfg.zarmon = ini.cfg.zarmon+boolGivemoney[0]
        zarmonsesia = zarmonsesia+boolGivemoney[0]
        azzarsesia = azzarsesia + 1
        ini.cfg.azzar = ini.cfg.azzar + 1      
                boxhkosesia = boxhkosesia + 1
        ini.cfg.boshkiini = ini.cfg.boshkiini + 1
        cfg_save()
        end
end 
      function deleteAll()
 ...
 boxhkosesia=0
 cfg_save()
 sampAddChatMessage('{006600}[Oil Tools]: {FFFFFF}Статистика сброшена', -1)
end  
  imgui.OnInitialize(function()
  decor()
...
lua_thread.create(function()
show_arz_notify('info', 'Oil Tool', "Перезагрузка....!", 500)
wait(0)
thisScript():reload()
end)
end
...
lua_thread.create(function()
show_arz_notify('info', 'Oil Tool', "Выключение....!", 500)
wait(0)
thisScript():unload()
end)
end
...
imgui.OnFrame(function() return UpdateWindow[0] end, function(player)
    ...
    imgui.CenterText(u8'У вас сейчас установлена версия хелпера ' .. u8(tostring(thisScript().version)) .. ".")
		...
		imgui.Separator()
		if imgui.Button(fa.CIRCLE_XMARK .. u8' Не обновлять ',  imgui.ImVec2(300 * ini.cfg.custom_dpi, 25 * settings.cfg.custom_dpi)) then
			UpdateWindow[0] = false
		end
		imgui.SameLine()
		if imgui.Button(fa.DOWNLOAD ..u8' Загрузить новую версию',  imgui.ImVec2(300 * ini.cfg.custom_dpi, 25 * ini.cfg.custom_dpi)) then
...
msg("Открыть главное меню: /oil")
    sampRegisterChatCommand('oil', function() mainMenu[0] = not mainMenu[0] end)
end	
...
function openLink(link)
	...
	if not ini.cfg.autofind_dpi then
	print('Применение авто-размера менюшек...')
	if isMonetLoader() then
		ini.cfg.custom_dpi = MONET_DPI_SCALE
	else
		...
		ini.cfg.custom_dpi = (width_scale + height_scale) / 2
	end
	ini.cfg.autofind_dpi = true
	print('Установлено значение: ' .. ini.cfg.custom_dpi)
	cfg_save()
end
