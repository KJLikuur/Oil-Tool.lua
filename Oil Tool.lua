...
if imgui.Checkbox(u8' Заработанные вирты', moneycfg) then settings.cfg.moneycfg = moneycfg[0] cfg_save()  end
	...
	imgui.CenterText('Заработано денег учётом охраны за все время: '..separator(moneyoherall))
	imgui.CenterText('Заработано денег учётом охраны зс сессию: '..separator(moneyoherallses))
  ...
  imgui.OnFrame(function() return OilStats[0] end, function(player)
    imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 8, sizeY / 1.7), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
      ...
      function sampev.onServerMessage(color, text)
          local wBar, proc = text:match('Цена (.+) нефтебочки увеличена на (%d+).+, поскольку у вашего охранника есть специальная характеристика')
